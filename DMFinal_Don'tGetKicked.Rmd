...
At the conclusion of running these regex operations, we delete the [Model] and [SubModel] variables.
...
The results show that GCV Earth perform better than the other models by a statiscally significant amount with a mean ROC score of 0.74595. There is a low pract_equiv score when comparing GCV to other models, which means the performance of the other models are not statistically similar to GCV Earth. Due to these reasons, we choose GCV Earth as our final model to use with the test set. 
...
We have selected GCVearth as the best performing model from the training and cross validation results. 
We have predicted the test data on this model. The model correctly predicted 1,492 Lemons ("Badbuy") out of 2,235 lemons (67%).The model also incorrectly predicted good buys as Lemons 5,229 out of 16,007 good buys (33%). The overall accuracy of predictions is 67.3%. We feel that these results would provide value to a car dealers when buying cars at an auction. Although the accuracy is not very high, the predictions could be used as an initial indicator whether a car may be a lemon or not. 


Implementation
This model could be integrated with a software application that would allow car dealers to send a request with specifc car details from an auction, so then the model could return a response whether it predicts the car to be a lemon or not. The user should use this response as an indicator to ask the auctioneer for more information or conduct additional inspection of the car before buying to reduce the risk of the car being a lemon. 
After car dealers have bought a car and determined whether or not it was a lemon, they should enter this new data in the application and the models should be re-trained with this additional data, so the models could continuously learn overtime and detect any new trends in the data to increase accuracy of it's predictions.
